...
TRANSLATIONS = {
    "pt-BR": {
        ...
        "close_behavior": "Ao fechar a janela",
        "ask_on_close": "Perguntar sempre",
        "minimize_to_tray": "Minimizar para bandeja",
        "exit_app": "Encerrar aplicativo",
        "tray_open": "Abrir",
        "tray_exit": "Sair",
        "tray_running": "O aplicativo continua em execução na bandeja do Windows.",
        "confirm_close_title": "Fechar aplicativo",
        "confirm_close_msg": "Escolha o que deseja fazer ao fechar a janela.",
        "close_now": "Fechar agora",
        "minimize_now": "Minimizar e continuar",
        "start_minimized": "Iniciar minimizado",
        "settings": "Configurações",
        "settings_description": "Idioma, comportamento ao fechar e opções de inicialização.",
        "appearance": "Preferências",
        "save_settings_close": "Salvar e fechar",
        "close_without_save": "Fechar sem salvar",
    },
    "en-US": {
        "app_title": "Crimson Desert Save Guardian",
        ...
        "ask_on_close": "Always ask",
        "minimize_to_tray": "Minimize to tray",
        "exit_app": "Exit application",
        "tray_open": "Open",
        "tray_exit": "Exit",
        "tray_running": "The application is still running in the Windows tray.",
        "confirm_close_title": "Close application",
        "confirm_close_msg": "Choose what you want to do when closing the window.",
        "close_now": "Close now",
        "minimize_now": "Minimize and continue",
        "start_minimized": "Start minimized",
        "settings": "Settings",
        "settings_description": "Language, close behavior and startup options.",
        "appearance": "Preferences",
        "save_settings_close": "Save and close",
...
class App(ctk.CTk):
    def __init__(self):
        ...
        self.close_action_name_var = ctk.StringVar(value="")
    ...
    def _center_child_window(self, window, width: int, height: int):
    ...
    def _bring_dialog_front(self, window):
        try:
            window.lift()
            window.focus_force()
    ...
    def _animate_dialog_in(self, window):
        try:
            ...
            def step(alpha=0.0):
                ...
                window.after(18, lambda: step(alpha))
            window.after(10, step)
        except Exception:
            pass

    def _create_overlay(self, dialog):
        try:
            ...
            overlay.lift()
            dialog.lift()
            dialog._overlay_ref = overlay
            dialog.bind("<Destroy>", lambda e: overlay.destroy() if overlay.winfo_exists() else None)
        except Exception:
            pass
    ...
    def _build_ui(self):
        ...
        # language selector
        self.settings_btn = ctk.CTkButton(self.left_scroll, text="", height=38, fg_color="#3b82f6", hover_color="#2563eb", command=self.open_settings_window)
        self.settings_btn.pack(fill="x", padx=10, pady=(6, 14))
    ...
    def open_settings_window(self):
        ...
        self._create_overlay(dialog)
        self._bring_dialog_front(dialog)
        self._animate_dialog_in(dialog)
        dialog.grab_set()

        temp_language = ctk.StringVar(value=LANGUAGE_OPTIONS.get(self.lang, list(LANGUAGE_OPTIONS.values())[0]))
        action_map = {
            "ask": self.tr("ask_on_close"),
            "minimize": self.tr("minimize_to_tray"),
            "exit": self.tr("exit_app"),
        }
        temp_close = ctk.StringVar(value=action_map.get(getattr(self.service.config, "close_action", "ask"), self.tr("ask_on_close")))
        temp_start_minimized = ctk.BooleanVar(value=bool(getattr(self.service.config, "start_minimized", False)))
        temp_backup_on_start = ctk.BooleanVar(value=bool(self.backup_on_start_var.get()))
        temp_only_if_changed = ctk.BooleanVar(value=bool(self.only_if_changed_var.get()))

        frame = ctk.CTkFrame(dialog, corner_radius=18)
        frame.pack(fill="both", expand=True, padx=16, pady=16)

        ctk.CTkLabel(frame, text=self.tr("settings"), font=ctk.CTkFont(size=22, weight="bold")).pack(anchor="w", padx=16, pady=(16, 6))
        ctk.CTkLabel(frame, text=self.tr("settings_description")).pack(anchor="w", padx=16, pady=(0, 14))

        body = ctk.CTkScrollableFrame(frame, fg_color="transparent")
        body.pack(fill="both", expand=True, padx=12, pady=(0, 12))

        ctk.CTkLabel(body, text=self.tr("language"), font=ctk.CTkFont(size=15, weight="bold")).pack(anchor="w", padx=8, pady=(6, 6))
        language_segmented = ctk.CTkSegmentedButton(
            body,
            values=list(LANGUAGE_OPTIONS.values()),
            variable=temp_language,
        )
        language_segmented.pack(fill="x", padx=8, pady=(0, 14))

        ctk.CTkLabel(body, text=self.tr("close_behavior"), font=ctk.CTkFont(size=15, weight="bold")).pack(anchor="w", padx=8, pady=(6, 6))
        close_segmented = ctk.CTkSegmentedButton(
            body,
            values=[self.tr("ask_on_close"), self.tr("minimize_to_tray"), self.tr("exit_app")],
            variable=temp_close,
        )
        close_segmented.pack(fill="x", padx=8, pady=(0, 14))

        ctk.CTkLabel(body, text=self.tr("appearance"), font=ctk.CTkFont(size=15, weight="bold")).pack(anchor="w", padx=8, pady=(6, 6))
        start_minimized_check = ctk.CTkCheckBox(body, text=self.tr("start_minimized"), variable=temp_start_minimized)
        start_minimized_check.pack(anchor="w", padx=8, pady=4)
        backup_on_start_check = ctk.CTkCheckBox(body, text=self.tr("backup_on_start"), variable=temp_backup_on_start)
        backup_on_start_check.pack(anchor="w", padx=8, pady=4)
        only_changed_check = ctk.CTkCheckBox(body, text=self.tr("only_if_changed"), variable=temp_only_if_changed)
        only_changed_check.pack(anchor="w", padx=8, pady=4)
        ...
        def save_and_close():
            selected_lang_name = temp_language.get()
            for code, name in LANGUAGE_OPTIONS.items():
                if name == selected_lang_name:
                    self.lang = code
                    break
            self.language_name_var.set(LANGUAGE_OPTIONS[self.lang])
            self.start_minimized_var.set(bool(temp_start_minimized.get()))
            self.backup_on_start_var.set(bool(temp_backup_on_start.get()))
            self.only_if_changed_var.set(bool(temp_only_if_changed.get()))
            self.close_action_name_var.set(temp_close.get())
            self.save_ui_config()
            self.apply_language()
            self.stop_tray_icon()
            dialog.destroy()

        ctk.CTkButton(btns, text=self.tr("save_settings_close"), command=save_and_close).grid(row=0, column=0, padx=(0, 6), sticky="ew")
        ctk.CTkButton(btns, text=self.tr("close_without_save"), fg_color="#444444", hover_color="#525252", command=dialog.destroy).grid(row=0, column=1, padx=(6, 0), sticky="ew")
    ...
    def get_close_action_code(self) -> str:
        current = self.close_action_name_var.get()
        if current == self.tr("minimize_to_tray"):
            return "minimize"
        if current == self.tr("exit_app"):
            return "exit"
        return "ask"

    def on_close_action_change(self, value: str):
        self.set_footer(f"{self.tr('close_behavior')}: {value}")

    def create_tray_image(self):
        img = Image.new("RGBA", (64, 64), (10, 26, 56, 255))
        draw = ImageDraw.Draw(img)
        draw.rounded_rectangle((8, 8, 56, 56), radius=14, fill=(30, 64, 175, 255))
        draw.rectangle((24, 18, 40, 46), fill=(255, 255, 255, 255))
        draw.rectangle((20, 26, 44, 30), fill=(255, 255, 255, 255))
        return img
    ...
    def ensure_tray_icon(self):
        if self.tray_icon is not None:
            return
        menu = pystray.Menu(
            pystray.MenuItem(self.tr("tray_open"), lambda icon, item: self.after(0, self.restore_from_tray), default=True),
            pystray.MenuItem(self.tr("backup_now"), lambda icon, item: self.after(0, self.manual_backup)),
            pystray.MenuItem(self.tr("start_auto_backup"), lambda icon, item: self.after(0, self.start_monitor)),
            pystray.MenuItem(self.tr("stop_auto_backup"), lambda icon, item: self.after(0, self.stop_monitor)),
            pystray.MenuItem(self.tr("tray_exit"), lambda icon, item: self.after(0, self.exit_from_tray)),
        )
        self.tray_icon = pystray.Icon("save_guardian_v8", self.create_tray_image(), self.tr("app_title"), menu)
    ...
    def hide_to_tray(self):
        ...
        self.set_footer(self.tr("tray_running"))
    ...
    def on_close_window(self):
        action = getattr(self.service.config, "close_action", self.get_close_action_code())
        if action == "minimize":
            self.hide_to_tray()
            return
        if action == "exit":
        ...
        dialog = ctk.CTkToplevel(self)
        dialog.title(self.tr("confirm_close_title"))
        ...
        top = ctk.CTkFrame(frame, corner_radius=12, fg_color="#2563eb", height=12)
        top.pack(fill="x", padx=16, pady=(16, 12))

        ctk.CTkLabel(frame, text=self.tr("confirm_close_title"), font=ctk.CTkFont(size=20, weight="bold")).pack(anchor="w", padx=16, pady=(4, 8))
        ctk.CTkLabel(frame, text=self.tr("confirm_close_msg"), justify="left", wraplength=430).pack(anchor="w", padx=16, pady=(0, 20))
        ...
        ctk.CTkButton(buttons, text=self.tr("close_now"), command=lambda: choose("exit")).grid(row=0, column=0, padx=(0, 6), sticky="ew")
        ctk.CTkButton(buttons, text=self.tr("minimize_now"), command=lambda: choose("minimize")).grid(row=0, column=1, padx=6, sticky="ew")
        ctk.CTkButton(buttons, text=self.tr("cancel"), fg_color="#444444", hover_color="#525252", command=lambda: choose("cancel")).grid(row=0, column=2, padx=(6, 0), sticky="ew")
    ...
    def load_config_to_ui(self):
        ...
        self.language_name_var.set(LANGUAGE_OPTIONS[self.lang])
        self.start_minimized_var.set(getattr(cfg, "start_minimized", False))
        action_map = {"ask": TRANSLATIONS[self.lang]["ask_on_close"], "minimize": TRANSLATIONS[self.lang]["minimize_to_tray"], "exit": TRANSLATIONS[self.lang]["exit_app"]}
        self.close_action_name_var.set(action_map.get(getattr(cfg, "close_action", "ask"), TRANSLATIONS[self.lang]["ask_on_close"]))
    ...
    def save_ui_config(self):
        cfg = Config(
            ...
            language=self.lang,
            close_action=self.get_close_action_code(),
    ...
    def on_tree_double_click(self, event=None):
        ...
        dialog.transient(self)
        dialog.resizable(False, False)
        self._center_child_window(dialog, 420, 220)
        self._create_overlay(dialog)
        self._bring_dialog_front(dialog)
        self._animate_dialog_in(dialog)
        dialog.grab_set()
    ...
    def show_custom_dialog(self, title: str, message: str, kind: str = "info"):
        ...
        dialog.transient(self)
        dialog.resizable(False, False)
        self._center_child_window(dialog, 460, 240)
        self._create_overlay(dialog)
        self._bring_dialog_front(dialog)
        self._animate_dialog_in(dialog)
        dialog.grab_set()
        ...
        top = ctk.CTkFrame(frame, corner_radius=12, fg_color=accent, height=12)
        top.pack(fill="x", padx=16, pady=(16, 12))
    ...
    def ask_custom_confirm(self, title: str, message: str) -> bool:
        ...
        dialog.transient(self)
        dialog.resizable(False, False)
        self._center_child_window(dialog, 500, 260)
        self._create_overlay(dialog)
        self._bring_dialog_front(dialog)
        self._animate_dialog_in(dialog)
        dialog.grab_set()
        ...
        top = ctk.CTkFrame(frame, corner_radius=12, fg_color="#d97706", height=12)
        top.pack(fill="x", padx=16, pady=(16, 12))
